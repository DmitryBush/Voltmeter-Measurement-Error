...
def Calculate_Resistance_Error(dU_HAst, U, dIck, I, R):
    return round(R * math.sqrt((dU_HAst / U) ** 2 + (dIck / I) ** 2))
...
def Calculate_Result_ResistanseError(R1, R2, R3, R4):
    return round(1 / math.sqrt(1 / (R1 ** 2) + 1 / (R2 ** 2) + 1 / (R3 ** 2) + 1 / (R4 ** 2)), 0)


persons = {'Data_Person1': np.array([[5.1, 0.2], [2.3, 0.05], [1.1, 0.05], [1.7, 0.05], [193, 5]]),
           'Data_Person2': np.array([[11.8, 0.2], [5.3, 0.2], [2.6, 0.05], [4.0, 0.20], [475, 50]]),
           'Data_Person3': np.array([[21.8, 0.5], [9.7, 0.2], [4.8, 0.20], [7.5, 0.20], [825, 50]]),
           'Data_Person4': np.array([[28.8, 0.5], [13, 0.5], [6.1, 0.2], [9.8, 0.2], [1100, 50]])}

resistors = {'resistors1': np.array([]),
             'resistors2': np.array([]),
             'resistors3': np.array([])}

error_resistors = {'resistors1': np.array([]),
                   'resistors2': np.array([]),
                   'resistors3': np.array([])}

middle_coefficient = {'middle_R1': np.array([]),
                      'middle_R2': np.array([]),
                      'middle_R3': np.array([])}
